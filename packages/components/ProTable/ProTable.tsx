...
>(<T extends Record<string, unknown>>(props: ProTableProps<T>, ref: React.Ref<ProTableActionType<T>>) => {
  ...
  const { instance, bindingProps, store, setFormInstance } = useProTable<T>({ ...props });
  ...
  // 注入 Form 实例到 useProTable（填充 instance.form 和 action.getFormInstance）
  useEffect(() => {
    setFormInstance(formRef.current ?? undefined);
  }, [setFormInstance, formRef.current]);

  // === 增强 instance.action（UI 层特有能力：弹窗、虚拟滚动、拖拽排序）===
  // 直接在 instance.action 上 mutate，确保 instance.action 与 ref 为同一对象
  useEffect(() => {
    // 保存原始 reset 引用，避免覆盖后循环调用
  ...
  // 暴露 instance.action 给 ref（与 useProTable 返回的 instance.action 是同一对象）
      ...
      {store.error ? (
        ...
        <>
          ...
          {viewMode === 'card' && cardMode ? (
            ...
            <div ref={virtualScrollContainerRef} style={virtualScroll ? { height: 400, overflow: 'auto' } : undefined}>
              {virtualScroll ? (
  ...
  return (
    <ProTableContext.Provider value={contextValue}>
      <RootProvider props={props}>
        <DataProvider store={store} formRef={formRef} action={instance.action} onDataSourceChange={onDataSourceChange}>
          <ColumnProvider
...
>(<T extends Record<string, unknown>>(props: ProTableProps<T>, ref: React.Ref<ProTableActionType<T>>) => {
  ...
  const { table: _omit, dialogConfig, ...rest } = props;
  void _omit;

  // 同步增强传入的 instance.action（确保用户侧的 instance 也获得弹窗能力）
  useEffect(() => {
    Object.assign(instance.action, {
      openDialog: ((config: Parameters<typeof openDialog>[0]) => {
        const defaultOpenConfig = dialogConfig?.open || {};
  ...
  }, [instance.action, dialogConfig]);

  // 将 instance 的状态作为 props 传入，ProTableStandalone 会通过 dataSource/loading/pagination 接管
  const mergedProps = {
    ...rest,
    dataSource: instance.dataSource,
