  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', gap: 8 }}>
        <button onClick={() => setFieldCount(prev => Math.max(1, prev - 1))}>
          减少字段
        </button>
        <button onClick={() => setFieldCount(prev => Math.min(5, prev + 1))}>
          添加字段
        </button>
      </div>
      <ProForm
...
function ConditionalForm() {
  ...
  return (
    <div>
      ...
      <ProForm
        schemas={formType === 'basic' ? basicSchemas : advancedSchemas}
        showButton
      />
    </div>
  );
