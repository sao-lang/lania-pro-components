 ...
 * 草稿持久化引擎（DraftEngine）
 *
 * 迁移自 ProForm/core/DraftEngine.ts
 * 为表单等场景提供草稿自动保存/恢复功能，支持多种存储策略：
 ...
 * @example
 * ```ts
 * import { DraftEngine, localStorageStrategy } from '@lania-pro-components/utils';
 *
 ...
 *
 * // 保存草稿
 * draftEngine.save({ name: 'Alice' });
 *
 * // 恢复草稿
 * const saved = draftEngine.load();
 * if (saved) console.log('草稿已恢复', saved.values);
 * ```
 */
...
export class DraftEngine {
  ...
  save(values: Record<string, unknown>): boolean {
    ...
    if (valuesStr === this.lastSavedValues) return false;
  ...
  autoSave(values: Record<string, unknown>): void {
    if (!this.config.enabled || this.config.autoSaveDelay <= 0) return;
...
/** 草稿状态 */
export type DraftStatus = 'none' | 'saved' | 'restored' | 'exists';

/**
 * 创建草稿引擎实例
 * 工厂函数，与 new DraftEngine() 等价
 */
